...
import { CircleChevronRight, FileIcon } from "lucide-react";
...
export const FileBreadcrumb = memo(
  ({ files, pathSegments = [], onFileSelect }: FileBreadcrumbProps) => {
            ...
            <div key={index} className="relative flex items-center">
              <DropdownMenu.Root open={isActive} modal={false}>
                <DropdownMenu.Trigger asChild>
                  ...
                  >
                    {isLast && <FileIcon className="w-4 h-4 text-white" />}

                    {segment}
                  </span>
                </DropdownMenu.Trigger>
                {index > 0 && !isLast && (
                  <CircleChevronRight className="w-4 h-4 text-white inline-block mx-1" />
                )}
                <AnimatePresence>
                  {isActive && (
                    <DropdownMenu.Portal>
